...
import type { Environment, Collection, History } from '../../src/types';

// In-memory storage for tests
const mockEnvironments: Map<string, Environment> = new Map();
const mockCollections: Map<string, Collection> = new Map();
const mockHistories: Map<string, History> = new Map();
  ...
  invoke: vi.fn(async (cmd: string, args?: any) => {
    switch (cmd) {
      case 'save_environment':
        mockEnvironments.set(args.env.id, args.env);
        return undefined;
      case 'get_environments':
        return Array.from(mockEnvironments.values());
      case 'save_collection':
        mockCollections.set(args.collection.id, args.collection);
        return undefined;
      case 'get_collections':
        return Array.from(mockCollections.values());
      case 'add_history':
        mockHistories.set(args.history.id, args.history);
        return undefined;
      case 'get_histories':
        const histories = Array.from(mockHistories.values());
        return args?.limit ? histories.slice(0, args.limit) : histories;
      default:
        return undefined;
    }
  }),
